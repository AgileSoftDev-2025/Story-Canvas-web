...
function HistoryContent() {
  ...
  const fetchUserProjects = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log('🔐 Unauthorized - token mungkin expired');
  ...
  // ✅ DELETE PROJECT menggunakan endpoint /api/projects/{project_id}/delete/
  const handleDelete = async (projectId: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:5173/api/projects/${projectId}/delete/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          setFilteredProjects(prev => prev.filter(project => project.project_id !== projectId));
  ...
  // ✅ RENAME PROJECT menggunakan endpoint /api/projects/{project_id}/update/
  const handleRenameSave = async (projectId: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:5173/api/projects/${projectId}/update/`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
            ...
            prev.map(project =>
              project.project_id === projectId
                ? {
                  ...project,
                  title: renameValue.trim(),
                  last_modified: data.data?.last_modified || new Date().toISOString()
                }
                : project
            )
          );
