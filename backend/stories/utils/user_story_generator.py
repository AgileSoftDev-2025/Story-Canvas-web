...
class UserStoryGenerator:
    ...
    def parse_json_output_userstories(self, output_text: str) -> Dict:
        """MAXIMUM AGGRESSIVE JSON parsing - EXACT SAME AS COLAB"""
        import json
        import re
        ...
        def extract_stories_line_by_line(text: str) -> List[Dict]:
            """Line-by-line extraction for malformed JSON - EXACT SAME AS COLAB"""
            stories = []
            lines = text.split('\n')
            i = 0
            ...
            while i < len(lines):
                ...
                if line.startswith('{') or '"text"' in line:
                    story_lines = []
                    brace_count = 0

                    # Collect until we have a complete object
                    for j in range(i, min(i + 20, len(lines))):  # Limit search to 20 lines
                        story_lines.append(lines[j])
                        brace_count += lines[j].count('{')
                        brace_count -= lines[j].count('}')

                        if brace_count == 0 and j > i:
                            # Try to parse this as a story object
                            story_text = ' '.join(story_lines)

                            # Extract field values using regex
                            text_match = re.search(r'"text"\s*:\s*"([^"]*)"', story_text)
                            role_match = re.search(r'"role"\s*:\s*"([^"]*)"', story_text)
                            feature_match = re.search(r'"feature"\s*:\s*"([^"]*)"', story_text)

                            if text_match:
                                story_text_content = text_match.group(1)
                                # If we have story text but no role, extract role from text
                                if not role_match:
                                    role_extract = re.search(r'As a ([^,]+),', story_text_content)
                                    role = role_extract.group(1) if role_extract else "User"
                                else:
                                    role = role_match.group(1)

                                if not feature_match:
                                    def extract_feature(action_text: str) -> str:
                                        if not action_text:
                                            return "General"
                                        action_lower = action_text.lower()
                                        if any(word in action_lower for word in ['train', 'teach', 'educate', 'learn']):
                                    ...
                                    feature = extract_feature(story_text_content)
                                else:
                                    feature = feature_match.group(1)

                                story = {
                                    "text": story_text_content,
                                    "role": role,
                                    "feature": feature,
                                    "acceptance_criteria": [
                                        f"The system allows {role} to complete the action successfully",
                                        f"Proper feedback is provided to {role}",
                                        f"The intended benefit is achieved"
                                    ],
                                    "priority": "Medium"
                                }
                                stories.append(story)
        ...
        def regex_story_extraction(text: str) -> List[Dict]:
            """Final fallback - brute force regex extraction - EXACT SAME AS COLAB"""
            stories = []

            # Ultra-permissive pattern
        ...
        # STRATEGY 4: Line-by-line story detection
        stories = extract_stories_line_by_line(text)
        if stories:
            print(f"      ✅ Line-by-line found {len(stories)} stories")
            return {"user_stories": stories}

        # STRATEGY 5: Final fallback - regex pattern matching
        stories = regex_story_extraction(text)
        if stories:
    ...
    def generate_comprehensive_user_stories(self, project_info: Dict, project_analysis: Dict, similar_patterns: List[Dict]) -> List[Dict]:
        ...
        for simple_role in simplified_roles:
            ...
            # SIMPLIFIED AND CLEANER PROMPT - EXACT SAME AS COLAB
            prompt = f"""
...
"""
    ...
    def generate_user_stories_for_project(self, project, rag_db=None):
        ...
        try:
            ...
            # Get similar patterns using RAG
            similar_patterns = []
            if rag_db:
